import { AsyncStorage } from 'react-native';

import { put, call } from 'redux-saga/effects'
import { ActionTypes } from '../constants/constants'
import { subscriptionPayment } from '../api/api'
import NavigationService from '../helpers/navigationService'
const TEST_TOKEN = 'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOjEzMDcyLCJpc3MiOiJodHRwOlwvXC9kZW1vLXN5bmVzdGhlc2lhLnppbWFsYWIuY29tXC9hdXRoIiwiaWF0IjoiMTU1MzE4MTI5OSIsImV4cCI6IjE1NTMyNjc2OTkiLCJuYmYiOiIxNTUzMTgxMjk5IiwianRpIjoiNjFlMDEyYzFhOWVmYjhjZjA2OThlNmY0ZWEwODNiMzIifQ.gmNdW-Ny21ueCaLsLYan62gYIqFWaPP57yhBE0GjBlw'
const PaymentSaga = function* (action) {
  const token = yield AsyncStorage.getItem('token');

  if (token != null) {
    const dataObject = yield call(subscriptionPayment, 'blablabla', { planId: action.payload.planId, stripeToken: 'tok_visa' });
    if (dataObject.status.success) {
      let tier = 'monthly'
      if (action.payload.planId === 'plan_DV5jpbVX2EW1rV') {
        tier = 'yearly'
      }
      yield put({
        type: ActionTypes.PAY_SUBSCRIPTION_SUCCESS,
        payload: {
          ...dataObject
        }
      })
      yield put({
        type: ActionTypes.SET_MODAL_TYPE,
        payload: {
          modalType: '7-dayTrial'
        }
      })
      yield put({
        type: ActionTypes.SET_SUBSCRIPTION_TYPE_SUCCESS,
        payload: tier
      })
      yield put({
        type: ActionTypes.OPEN_SUCCESS_MODAL,
      })
      NavigationService.navigate('User')
    }
    else {
      yield put({
        type: ActionTypes.PAY_SUBSCRIPTION_FAIL
      })
      yield put({
        type: ActionTypes.SET_MODAL_TYPE,
        payload: {
          modalType: '7-dayTrial'
        }
      })
      yield put({
        type: ActionTypes.OPEN_ERROR_MODAL,
      })
    }
  }
}

export default PaymentSaga
